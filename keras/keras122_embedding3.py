...
pad_x = keras.preprocessing.sequence.pad_sequences(x, padding = 'pre', value = 0)    # value = 1.0
print(pad_x)                                                                         #12행 5열
'''여기까지 하면 데이터가 준비됨'''
...
model = Sequential()
#model.add(Embedding(word_size, 100, input_length= 5 ))   #전체 단어의 숫자, 히든, 인풋값(12,5이므로)

#model.add(Embedding(25, 10, input_length= 5 ))   #Q1(none, 5, 10)  => ??


model.add(Embedding(25, 10 ))   
model.add(LSTM(3))
#☆☆☆embedding과 lstm을 같이 쓰면 input_length를 쓰지 않아도 된다. Q3.

model.add(Flatten())                                     #Q2.flatten을 하면 dense형이 가능하다는데 무슨 소리임..?
...
acc = model.evaluate(pad_x, labels)[1]   #[1] ?? =>
print("acc: ", acc)
