...
#즉 이 과정을 통해서 samsung은 과거의 3개의 데이터로 1개를 훈련하고 hite는 3행으로 그 다음의 시가를 예측하는 모델을 만들수 있다. 
...
x3 = x1
x4 = x2
print(x3[-1,:].shape)           # (3, 1)
print(x4[-1,:].shape)           #(3, 5)
...
x3[-1] = x3[-1].reshape(1, 3 )
x4[-1,: ] = x4[-1, :].reshape(1, 15)
y2_pred = model.predict([x3[-1], x4[-1,:]])
